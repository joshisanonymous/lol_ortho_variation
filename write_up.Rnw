...
\begin{document}
  ...
  \label{sec:results}
    ...
    \begin{figure}[tbhp]
      ...
      <<div_individual_community, fig.height = 3.5, fig.width = 3.5>>=
      @
    \end{figure}
    ...
    Other than community 2265 where Rithanya made up the bulk of the total tokens, the distribution for communities 1291 and 1032 are quite similar: \orth{lol} is the clear mode, \orth{LOL} is still pretty frequent, and \orth{Lol} has an appreciable frequency.
    There were, of course, many other variants produced, but following Zipf's law, no other variant had more than five tokens in the entire corpus and most others had only one, so they were excluded from these charts to make the charts more readable.
    \begin{figure}[tbhp]
      \centering
