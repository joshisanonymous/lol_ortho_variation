...
@
  ...
  \section{Results}
    ...
    \subsection{Social constraints}
      ...
      However, (lol) was rarely used in these two communities as only \Sexpr{length(unique(lol[lol$Community == "799",]$User))} and \Sexpr{length(unique(lol[lol$Community == "2067",]$User))} members produced (lol) at all in each, respectively.
      ...
      The mean number of tokens of (lol) produced by users is \Sexpr{round(mean(table(lol$User)), 2)}, and the median is \Sexpr{median(table(lol$User))}, suggesting that indeed most users produced very few tokens.
      Filtering out users who produced fewer than 10 tokens yields \num{\Sexpr{nrow(lolActive)}} tokens from \Sexpr{length(usersActive)} users.
