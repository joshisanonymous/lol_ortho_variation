...
\begin{document}
  ...
  \section{Introduction}
    ...
    \begin{enumerate}
      \item \Sexpr{lol[lol$id.occurrence == 840, "utilisateur"]}\footnote{
      ...
      }: \Sexpr{lol[lol$id.occurrence == 840, "texte"]} \label{sent:lol}
      \item \Sexpr{lol[lol$id.occurrence == 3071, "utilisateur"]}: \Sexpr{lol[lol$id.occurrence == 3071, "texte"]} \label{sent:LOL}
      \item \Sexpr{lol[lol$id.occurrence == 2959, "utilisateur"]}: \Sexpr{lol[lol$id.occurrence == 2959, "texte"]} \label{sent:lololol}
    \end{enumerate}
    ...
    \subsection{Previous work on (lol)}
      ...
      \subsubsection{As an orthographic variable}
        ...
        \begin{enumerate}[resume]
          \item \Sexpr{lol[lol$id.occurrence == 4289, "utilisateur"]}: \Sexpr{lol[lol$id.occurrence == 4289, "texte"]} \label{sent:looool}
        \end{enumerate}
  ...
  \section{Methods}
    ...
    \subsection{Data collection}
      ...
      Fortunately, the original spellings were stored in the corpus, and by far the most common lexical item was \lexi{lol}, so filtering out unwanted lexical items resulted in a final corpus with \Sexpr{length(levels(factor(lol$communaute)))} communities, \num{\Sexpr{nrow(lol)}} tokens of the orthographic variable (lol), and \Sexpr{length(levels(factor(lol$lol)))} spelling variants for (lol).
    ...
    \subsection{Coding}
      ...
      Of special note when interpreting why particular orthographic variants appear is the presence of auto-complete systems on both smartphones and computers.
      ...
      While this lack is distinction between what is auto-completed and what is not could introduce a potentially significant methodological issue, there are two important reasons to believe that the issue is not a large one.
      First, auto-complete systems are sometimes generated by the typing habits of the user.
      If a user always manually types \orth{LOL}, their auto-complete system may very well start correcting any spelling of (lol) to the fully capitalized variant.
      ...
      Second, the most likely candidate for auto-completion is \orth{Lol} at the beginning of a sentence, but this variant is easily the least frequent of the three top variants, as will be evident in the \nameref{sec:results} section below.
      ...
      \subsubsection{Social variables}
        ...
        For instance, \textcite{ilbury_sassy_2020} targeted openly gay men from the south of England in his study of Twitter, but he analyzed a only ten users as opposed to the \Sexpr{length(unique(lol$utilisateur))} users in my corpous.
...
@
      ...
      \subsubsection{Pragmatic variables}
        ...
        No preprosessing of the corpus was done before sending it to this classifier other than removing \lexi{lol} from the dictionary that the classifier uses.
  ...
  \section{Results}
    ...
    \begin{figure}[tbhp]
      \centering
      \caption{Diversity according to PageRank percentile within their communities for users who produced at least 10 tokens of (lol)}
      \label{fig:div_by_prperc_active}
